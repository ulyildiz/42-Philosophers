...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/22 11:53:39 by ulyildiz          #+#    #+#             */
/*   Updated: 2024/04/22 11:53:39 by ulyildiz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_status(t_flags status, int i, t_dining *tbl)
{
	...
	if (status == R_FORK && checking_flag(&tbl->waiting,
			&tbl->d_or_a) == ALIVE)
		printf("|%lu| - %d. philosopher's has taken a fork.(right)\n", time,
			i);
	else if (status == L_FORK && checking_flag(&tbl->waiting,
			&tbl->d_or_a) == ALIVE)
		printf("|%lu| - %d. philosopher's has taken a fork.(left)\n", time, i);
	else if (status == SLEEPING && checking_flag(&tbl->waiting,
			&tbl->d_or_a) == ALIVE)
		printf("|%lu| - %d. philosopher's sleeping.\n", time, i);
	else if (status == EATING && checking_flag(&tbl->waiting,
			&tbl->d_or_a) == ALIVE)
		printf("|%lu| - %d. philosopher's eating.\n", time, i);
	else if (status == THINKING && checking_flag(&tbl->waiting,
			&tbl->d_or_a) == ALIVE)
